...
import static com.bs.themebridge.util.ThemeConstant.OPERATION_CUSTOMERDETAILSEARCH;
...
public class CustomerCustomerDetailsAdapterImpl extends ServiceAdapterImpl{

	private final static Logger logger = Logger.getLogger(CustomerCustomerDetailsAdapterImpl.class);

	@Override
	public String process(String inputXml) throws ClassNotFoundException {
		logger.debug("Request enters CustomerCustomerDetails Adapter");
	...
	public static String getErrorResponse(String errorMessage) {
		...
		if (ValidationsUtil.isValidString(errorMessage)) {
			responseHeader = ResponseHeaderUtil.failedResponse(SERVICE_CUSTOMER, OPERATION_CUSTOMERDETAILSEARCH, "", "",
					errorMessage + " (IM)");
	...
	public AdapteeInterface getAdapteeInstance(String inputXml) {
		...
		AdapteeInterface implementationClass = AdapteeUtil.getAdapteeInstance(inputXml,
				AdapteeImplementation.getCustomercustomerdetailsadaptee());
